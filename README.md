...
For manual launch, start Taxi Cam from the Start menu (or tray) while MSFS is at the main menu, before loading a flight. You can leave **Auto-connect** on, or turn it off and use **Connect** after MSFS is up. If attach stalls, use **Reconnect / Reset**; it does not stop MSFS or overwrite calibration. Starting after the cockpit displays already exist can leave PFD detection incomplete. If this happens, keep Taxi Cam running and restart the flight; this recovered the observed iniBuilds A380 case. See [startup timing](docs/reshade-compatibility.md#startup-timing).
