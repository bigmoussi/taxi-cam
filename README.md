...
For manual launch, start Taxi Cam from the Start menu (or tray) while MSFS is at the **main menu**, before loading a flight. You can leave **Auto-connect** on, or turn it off and use **Connect** at the main menu. If you Connect after the aircraft has already loaded, PFD texture lists can stay empty until you **Restart Flight** with Taxi Cam still connected. If attach stalls, use **Reconnect / Reset**; it does not stop MSFS or overwrite calibration. See [startup timing](docs/reshade-compatibility.md#startup-timing).
