...
$embedded = @('procedure WriteUninstallScripts(Directory: String);', 'begin', "  ForceDirectories(Directory + '\standalone');")
foreach ($item in @(@('runtime.ps1', $RuntimeScript), @('uninstall-native.ps1', $UninstallScript), @('standalone\exe_xml.ps1', $ExeXmlScript))) {
