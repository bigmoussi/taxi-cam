...
param([Parameter(Mandatory=$true)][string]$Output, [Parameter(Mandatory=$true)][string]$RuntimeScript,
    ...
    [string]$SettingsScript = (Join-Path $PSScriptRoot 'settings.ps1'))
$ErrorActionPreference = 'Stop'
$embedded = @('procedure WriteUninstallScripts(Directory: String);', 'begin', '  ForceDirectories(Directory);')
foreach ($item in @(@('runtime.ps1', $RuntimeScript), @('uninstall.ps1', $UninstallScript), @('exe_xml.ps1', $ExeXmlScript), @('settings.ps1', $SettingsScript))) {
