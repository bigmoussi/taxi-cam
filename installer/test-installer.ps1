...
& $compiler "/DPayloadDir=$payload" "/DInternalDir=$testRoot" "/DRuntimeScript=$runtime" "/DAppVersion=$($receipt.version)" "/DBuildNumber=$($receipt.buildNumber)" '/DInstallerTest=1' '/DOutputBase=isolated-setup' "/O$testRoot" (Join-Path $PSScriptRoot 'taxi-cam.iss') *> (Join-Path $testRoot 'compiler.log')
...
$iss = Get-Content -Raw -LiteralPath (Join-Path $PSScriptRoot 'taxi-cam.iss')
