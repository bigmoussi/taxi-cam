...
$outputDirectory = Join-Path $repoRoot 'build/tests/hooks'
...
& $compiler @common '-fno-exceptions' '-c' (Join-Path $repoRoot 'src/hooks/observer_hook.cpp') '-o' $hookObject
if ($LASTEXITCODE -ne 0) { throw 'Observer hook failed to compile.' }
& $compiler '-c' (Join-Path $repoRoot 'src/hooks/observer_thunk.S') '-o' $thunkObject
...
& $compiler @common '-fno-exceptions' '-DTAXI_ENGINE_HOOK_VALIDATION' '-c' (Join-Path $repoRoot 'src/hooks/observer_hook.cpp') '-o' $faultHookObject
