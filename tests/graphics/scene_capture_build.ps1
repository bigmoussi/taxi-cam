...
$captureRoot = $repoRoot
$captureBuild = Join-Path $captureRoot 'build'
$captureCompiler = Join-Path (Get-TaxiToolchain $repoRoot) 'clang++.exe'
...
& $captureCompiler @captureFlags -fno-exceptions -c (Join-Path $captureRoot 'src/graphics/scene_capture_d3d12.cpp') -o $captureObject
if ($LASTEXITCODE -ne 0) { throw 'Capture production compilation failed.' }
& $captureCompiler @captureFlags -static (Join-Path $repoRoot 'tests/graphics/scene_capture_main.cpp') $captureObject -ld3d12 -ldxgi -ldxguid -o $captureExecutable
