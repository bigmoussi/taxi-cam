...
int main() {
  ...
  for (const auto* profile : profiles::Catalog) {
    ...
    if (profile->id == profiles::AerosoftA346.id) {
      ...
      assert(std::strcmp(profile->pfd_labels[0], "CaptPFD") == 0 && std::strcmp(profile->pfd_labels[1], "CoND") == 0);
      ...
      // panel.cfg CaptPFD 9,470,750,750 (live-verified captain display) and CoND 769,1230,750,750.
