...
& $compiler @flags (Join-Path $PSScriptRoot 'view_material_inventory.cpp') (Join-Path $PSScriptRoot 'view_material_inventory_test.cpp') '-o' (Join-Path $output 'view-material-test.exe')
if ($LASTEXITCODE -ne 0) { throw 'View material regression compile failed.' }
$sources = @('view_material_live.cpp','view_material_inventory.cpp','image_inventory.cpp','aircraft_inventory.cpp') | ForEach-Object { Join-Path $PSScriptRoot $_ }
$sources += @('code_contract.cpp','verified_profile.cpp') | ForEach-Object { Join-Path (Join-Path $nativeRoot 'native-camera') $_ }
