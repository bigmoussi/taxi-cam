 ...
 public:
  ...
  bool initialize(std::string& error) {
    ...
    if (size == 0 || size >= path.size()) {
      error = "Could not locate the inventory executable to find the pinned LLVM library.";
      return false;
    }
    const auto native_root = std::filesystem::path(std::wstring(path.data(), size)).parent_path().parent_path().parent_path();
    auto library_path = native_root / L"build/deps/llvm-mingw/llvm-mingw-20260908-ucrt-x86_64/bin/libLLVM-23.dll";
