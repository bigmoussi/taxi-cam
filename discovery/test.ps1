...
$nativeRoot = Split-Path -Parent $discoveryRoot
$compiler = Join-Path $nativeRoot 'build/deps/llvm-mingw/llvm-mingw-20260908-ucrt-x86_64/bin/clang++.exe'
if (-not (Test-Path -LiteralPath $compiler)) {
  throw 'Run the parent native-probe bootstrap first; its pinned compiler is required.'
}
$outputDirectory = Join-Path $discoveryRoot 'build'
...
& $compiler @common (Join-Path $discoveryRoot 'image_inventory.cpp') (Join-Path $discoveryRoot 'reference_inventory.cpp') (Join-Path $discoveryRoot 'inventory_test.cpp') '-o' $testExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'image_inventory.cpp') (Join-Path $discoveryRoot 'reference_inventory.cpp') (Join-Path $discoveryRoot 'call_reference_test.cpp') '-o' $callTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'image_inventory.cpp') (Join-Path $discoveryRoot 'pointer_inventory.cpp') (Join-Path $discoveryRoot 'pointer_inventory_test.cpp') '-o' $pointerTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'image_inventory.cpp') (Join-Path $discoveryRoot 'guard_inventory.cpp') (Join-Path $discoveryRoot 'guard_inventory_test.cpp') '-o' $guardTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'command_list_inventory.cpp') (Join-Path $discoveryRoot 'command_list_inventory_test.cpp') '-o' $commandListTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'callee_prefix.cpp') (Join-Path $discoveryRoot 'callee_prefix_test.cpp') '-o' $calleeTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'service_inventory.cpp') (Join-Path $discoveryRoot 'service_inventory_test.cpp') '-o' $serviceTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'static_literals.cpp') (Join-Path $discoveryRoot 'static_literals_test.cpp') '-o' $literalTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'static_numeric.cpp') (Join-Path $discoveryRoot 'static_numeric_test.cpp') '-o' $numericTestExecutable
...
& $compiler @common (Join-Path $nativeRoot 'native-camera/code_contract.cpp') (Join-Path $nativeRoot 'native-camera/code_contract_test.cpp') '-o' $codeTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'aircraft_inventory.cpp') (Join-Path $discoveryRoot 'aircraft_inventory_test.cpp') '-o' $aircraftTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'slot_prefix.cpp') (Join-Path $discoveryRoot 'slot_prefix_test.cpp') '-o' $slotTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'import_slot.cpp') (Join-Path $discoveryRoot 'import_slot_test.cpp') '-o' $importTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'rtti_metadata.cpp') (Join-Path $discoveryRoot 'rtti_metadata_test.cpp') '-o' $rttiTestExecutable
...
& $compiler @common (Join-Path $discoveryRoot 'image_inventory.cpp') (Join-Path $discoveryRoot 'reference_inventory.cpp') (Join-Path $discoveryRoot 'pointer_inventory.cpp') (Join-Path $discoveryRoot 'guard_inventory.cpp') (Join-Path $discoveryRoot 'static_literals.cpp') (Join-Path $discoveryRoot 'service_inventory.cpp') (Join-Path $discoveryRoot 'aircraft_inventory.cpp') (Join-Path $discoveryRoot 'slot_prefix.cpp') (Join-Path $discoveryRoot 'callee_prefix.cpp') (Join-Path $discoveryRoot 'import_slot.cpp') (Join-Path $discoveryRoot 'rtti_metadata.cpp') (Join-Path $discoveryRoot 'command_list_inventory.cpp') (Join-Path $discoveryRoot 'llvm_decoder.cpp') (Join-Path $discoveryRoot 'static_numeric.cpp') (Join-Path $nativeRoot 'native-camera/code_contract.cpp') (Join-Path $nativeRoot 'native-camera/activation_mask.cpp') (Join-Path $nativeRoot 'native-camera/verified_profile.cpp') (Join-Path $nativeRoot 'native-camera/source_view.cpp') (Join-Path $discoveryRoot 'main_module_inventory.cpp') (Join-Path $discoveryRoot 'main.cpp') '-ladvapi32' '-o' $inventoryExecutable
