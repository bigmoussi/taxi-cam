...
$managerCompiler = Join-Path $managerBuild 'deps/llvm-mingw/llvm-mingw-20260908-ucrt-x86_64/bin/clang++.exe'
...
foreach ($managerSource in @('scene_capture_manager', 'scene_capture_d3d12', 'scene_handoff', 'scene_source_state')) {
  ...
  & $managerCompiler @managerFlags @managerExceptionFlags -c (Join-Path $PSScriptRoot "$managerSource.cpp") -o $managerObject
...
& $managerCompiler @managerFlags -c (Join-Path $managerRoot 'engine-hook/queue_submit_observer.cpp') -o $managerQueueObject
...
& $managerCompiler @managerFlags -c (Join-Path $managerRoot 'engine-hook/render_boundary_observer.cpp') -o $managerBoundaryObject
...
& $managerCompiler @managerFlags -fno-access-control -static (Join-Path $PSScriptRoot 'scene_queue_tail_test.cpp') @managerObjects $managerQueueObject -ld3d12 -ld3dcompiler -ldxgi -ldxguid -o $tailExecutable
...
& $managerCompiler @managerFlags -static (Join-Path $PSScriptRoot 'scene_capture_manager_test.cpp') @managerObjects -ld3d12 -ld3dcompiler -ldxgi -ldxguid -o $managerExecutable
