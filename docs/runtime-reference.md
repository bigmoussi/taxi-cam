...
| **Show notifications** | Tray menu; on by default, saved as `[messages] notifications` in `settings.ini` (an older `in_simulator` key is read and replaced on the next save). Only bridge events that need the pilot's attention appear as Windows tray notifications from the companion: simulator build unsupported, presentation stalled (watchdog trip), camera startup failed (cameras stopped and will not retry) and native hook failure storm (disarmed for the session). Routine changes (connected, cameras ready, disconnected, cameras disarmed/resumed after a trip, above 60 knots, aircraft mismatch, capture paused) never toast; they stay in the companion status line, the tray tooltip and `bridge.log`. AppUserModelID `TaxiCam.Companion`; status-line and taskbar flash if the OS hides the banner. The bridge only publishes toast events over IPC and limits repeats per event and to four in 20 s. Nothing is drawn inside the simulator. |
...
Renderer fault evidence uses a separate maximum of 16 records of 8 KiB each (earlier 4 KiB records remain valid candidates), plus one `renderer-fault-code-<pid>.bin` code capture that each session replaces. Before creating a new session record, the bridge removes eligible old records outside the exception handler. Active or locked records are preserved; if there is no safe capacity, new fault evidence is declined. Installer diagnostics replace the previous installation report rather than appending indefinitely.
...
The frame-rate setting limits activation opportunities for each camera. Each opening is followed by a closed camera-manager interval. The range is 5–60 (minimum 5); install and the shipped default use 10. At low simulator update rates, opening and closing could therefore require work on every manager update at higher chosen rates. The bridge therefore caps the rate from the measured camera-manager update rate: feeds × rate stays at or below a third of the updates per second. For example, two feeds at 30 updates/s run at 5, at 45 updates/s at 7 and at 60 updates/s at 10. The cap is measured over 2 s windows and changes only when two consecutive windows agree. It never goes below 5 and never raises the saved rate. The log reports it as `frame_cap=` with `rate_limit=frame_rate`, and the companion shows "limited by the simulator frame rate". Settings do not guarantee completed image FPS. Compare render-thread, presented-frame and GPU timings with TAXI off/on at the same cockpit view when investigating stutter; loaded hooks retain essential tracking while OFF, so binary comparisons are also needed to measure changes in their standing overhead.
...
The log's `queries` and `query_ms` cover actual calls to the instrumented memory metadata APIs, including the fresh AA writable-span check. `allocation_queries`, `page_queries` and `region_queries` distinguish allocation information, page working-set information and legacy `VirtualQueryEx` calls; they sum to `queries`. A page query can validate a batch. Region queries include default image checks and private/image-page fallbacks. The exact AA flag reads also contribute to `read_ms`. `reads` and `read_bytes` count the exact `ReadProcessMemory` calls and requested bytes of the same inspection. These are last serviced callback metrics, not FPS or whole-session averages; they remain stale while inspection is idle. AA requires one private allocation and current `PAGE_READWRITE` protection for the complete 16-byte flag span; its writable check retains no cached proof across the operation.
...
`Hook timing` records measure CPU time that the bridge's hooks spend on the calling simulator thread. They are always enabled and are written with each periodic status block. Each value covers the interval since the previous record; the first record after bridge start is a baseline and is not written.

- The first line has `interval_ms`, the TSC rate used for conversion (`tsc_mhz`), the per-call measurement floor (`floor_ns`), and total bridge self time (`self_ms`). `updates` counts camera-manager updates in the interval, and `self_us_per_update` divides the self time by them. Updates approximate simulator frames only while the camera runtime is enabled.
- Each hook site is reported as `site=calls/self_us/max_us`. The sites are `state` (command-list state setters, including render targets and root bindings), `draw`, `barrier`, `render_pass`, `copy`, `close`, `reset`, `execute` (queue submission), `device` (resource, view, descriptor-copy and list creation) and `camera_manager` (the engine camera-manager update hook, including its inspection).
- Self time excludes the time spent inside the original runtime function when that call reaches the runtime directly. Chained bridge hooks on the same call count once, under the outer hook's site.
- `state`, `draw`, `barrier` and `device` run millions of times per second. Timing each of those calls cost about 29 ns, as much as the hooks' own work. These sites therefore time one call in 16, chosen at random, and multiply that call's time by 16. Their call counts stay exact, and their `max_us` is the largest sampled call. All other sites time every call.
- A timed call also records its own timestamp reads, about 15 ns. The bridge measures this `floor_ns` when it takes the baseline and removes `calls × floor_ns` from every site and thread total.
- Bounded-lock waits are grouped by budget as `wait100`, `wait500`, `wait1000`, `wait5000` and `wait_other`, each reported as `waits/wait_us/max_us/expired`. A wait is counted only when the first `try_lock` fails, including waits that later acquire the lock. Wait time is also part of the enclosing hook's self time.
- `Hook timing threads:` lists the twelve threads with the most self time, plus any other thread whose name contains `Main`, as `tid:name=self_us/wait_us [site:calls/us,site:calls/us]`. The name is the thread description set by the simulator. It is `unnamed` when there is none, and `shared` for threads beyond the 128 per-thread slots.

Compare these records with TAXI off and on, at the same view and location. With sampling, the measurement adds about 3 ns to a sampled-site call and about 29 ns to other hooked calls.
...
### Output admission before a render gate opens

Camera readiness (`ready=`) covers the entry, view, Node and Camera chain. It says nothing about the view's output chain (material → Bitmap → record → resource), which the renderer binds without a null check when a gate is open. The bridge therefore opens a render gate only for a view whose requested pane output was observed in the same inspection: mode 2, a present resource member, all three view size pairs at the profile pane, and the Bitmap sized to that pane. The status line reports this as `outputs=a/b`; a view that is camera-ready but output-refused shows `inspection=output_unavailable` and its gate stays closed while the pair is retained.

After creation, the closed-gate resize phase calls the native output routine once per view and then stays pending, with both gates closed, until that pane output is observed for both views (`output_waits=` counts those observer updates, bounded by twelve). An exhausted wait is a startup failure that follows the existing guarded retirement path with its cause in the log. Pooled views reused after native release keep the previous entry's material state; this admission does not allocate, retain or prove GPU completion, and it does not establish the renderer's own contract for that state. A 19/09 reproduction on a build with this admission crashed with `outputs=1/1` at `Prewarm phase=warming`, so a present pane output is necessary but not sufficient.

### Render-target record admission

The captured 1.9.12.0 code around the retained fault shows what the renderer actually binds: for each texture `T` (`Bitmap+88`) the `SetRenderTargets` replay resolves a render-target record as `[T+0x48] ? [+8] ? [index]`, else `[T+0x40]`, stores it in its slot and increments the bound-target count only when the record is non-null; the binder then walks the first `count` slots with no null check. A texture without a record in slot 0, with a populated later slot, is therefore dereferenced as null. The owned-view inspection now reads that resolution (subresource 0) for the diffuse (520), add-diffuse (664) and depth-stencil (712) slots inside the same bounded, reread trace, and the retention log reports it as `rt=` with three hex digits per view (1 Bitmap, 2 texture, 4 render-target record; e.g. `rt=707/707`). A gate opens only when the diffuse texture has its record; otherwise the view shows `inspection=rt_record_absent`, the gate is held closed (`rt_refusals=` consecutive pulses, `rt_holds=` cumulative) and, after twelve consecutive refused pulses, the pair is retired closed through the existing guarded path. The initial closed-gate output wait includes the record. These are reads of `T` and its record pointers only; no record, view or Bitmap is written, created or bound by the bridge.

### Pooled-view flag restoration

The bridge's only persistent write to a pooled view's flag words is clearing `P+48` bit 31 (`ToggleVpEffectAA`'s bit) on its own mode 2 views. Pool views keep `P+48` across entries and the captured setup only ORs/ANDs other bits, so a slot this bridge used would otherwise enter its next entry's setup with a flag state no engine path produces. The bridge now records each view it cleared and restores the bit before the slot's next creation (only a view still present in the fresh pool, association-free, not queued or marked for release, gate bit set) and before its own native erase. The retention log reports `flags=` per view together with `aa_restores=`, `aa_restore_failures=` and `aa_cleared_pending=`. This hands back state the bridge changed; it is not established as the cause of the aircraft-swap crash.

### Renderer fault site code capture

Because the store executable is not readable from a user account, the bridge writes one read-only copy of the loaded image's code around the retained RenderThreadProc fault instruction at arm time: it locates the instruction sequence in the executable sections and writes `renderer-fault-code-<pid>.bin` (a small header, then up to 128 KiB before and 768 KiB after the fault, covering frames 0-12 of the retained callstack) to `%LOCALAPPDATA%\Taxi Cam`, replacing the previous session's file. The log records `Renderer fault site code capture: written= fault_rva= window_rva= window_bytes=`. The vectored fault record also accepts that located RVA, so 1.9.12.0 sessions now produce completed register records. Version 2 of that 8 KiB record adds, at fault time and only after `VirtualQuery` shows a committed readable range, the render context's output-merger state (`RBX+0x79a0..0x7a40`), its bound-target count (`+0x7910`) and the first 0x100 bytes of the slot-1 render-target record (`evidence_flags` bits 0, 2 and 1). No engine call or image write is involved.
