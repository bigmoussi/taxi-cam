...
| **Show notifications** | Tray menu; on by default, saved as `[messages] notifications` in `settings.ini` (an older `in_simulator` key is read and replaced on the next save). Bridge events (connected, cameras ready, disconnected, simulator unsupported, presentation stalled/resumed, cameras disarmed, above 60 knots, aircraft mismatch, camera startup failed, capture paused, hook storm) appear as Windows tray notifications from the companion (AppUserModelID `TaxiCam.Companion`; status-line and taskbar flash if the OS hides the banner). The bridge only publishes them over IPC and limits repeats per event and to four in 20 s. Nothing is drawn inside the simulator. |
