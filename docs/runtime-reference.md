...
Renderer fault evidence uses a separate maximum of 16 records of 4 KiB each, plus one `renderer-fault-code-<pid>.bin` code capture that each session replaces. Before creating a new session record, the bridge removes eligible old records outside the exception handler. Active or locked records are preserved; if there is no safe capacity, new fault evidence is declined. Installer diagnostics replace the previous installation report rather than appending indefinitely.
...
| Empty PFD texture list after starting Taxi Cam inside a loaded flight | Late attach can miss Create*/CreateRTV. The bridge keeps learning display textures on later barrier and copy use until the profile's display set is present: eight iniBuilds A380 displays, or the iniBuilds A350 three-surface one-mip typeless group. Five-mip A350 auxiliaries do not close that scan or win automatic selection. A nonempty complete set still gets ten seconds to associate render-target views. Restart eligibility when the aircraft profile is applied. If the list stays empty after those textures are in use, investigate display dimensions and formats. |
...
Because the store executable is not readable from a user account, the bridge writes one read-only copy of the loaded image's code around the retained RenderThreadProc fault instruction at arm time: it locates the instruction sequence in the executable sections and writes `renderer-fault-code-<pid>.bin` (a small header, then up to 128 KiB either side) to `%LOCALAPPDATA%\Taxi Cam`, replacing the previous session's file. The log records `Renderer fault site code capture: written= fault_rva= window_rva= window_bytes=`. The vectored fault record also accepts that located RVA, so 1.9.12.0 sessions now produce completed register records. No engine call or image write is involved.
