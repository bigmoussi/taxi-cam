...
| Empty PFD dropdowns after **Auto-connect Off** → load flight → **Connect** | Same late-attach miss: Refresh cannot invent textures created before the bridge hooked `Create*Resource`. Keep Taxi Cam attached and use **Restart Flight**, or Connect at the main menu before loading. Distinct from auto both-lost rediscovery (issue #43 / PR #46). |
