...
After creation, the closed-gate resize phase calls the native output routine once per view and then stays pending, with both gates closed, until that pane output is observed for both views (`output_waits=` counts those observer updates, bounded by twelve). An exhausted wait is a startup failure that follows the existing guarded retirement path with its cause in the log. Pooled views reused after native release keep the previous entry's material state; this admission does not allocate, retain or prove GPU completion, and it does not establish the renderer's own contract for that state.
