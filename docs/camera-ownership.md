...
The camera ownership components in `src/camera/` manage entry pairs through injected callbacks. The simulator adapter in `probe.cpp` supplies those callbacks after checking its code, ABI, thread, manager lifetime and removal contracts. The ownership components themselves contain no simulator addresses, module discovery, hook installation or rendering implementation. Tests in `tests/camera/` use synthetic readers and callbacks.
...
```powershell
& .\tests\camera\ownership_test.ps1
```

The script uses the compiler pinned in `dependencies.json`, with C++20, `-O2 -Wall -Wextra -Werror`. It creates the library and test executables under `build/tests/camera/ownership/`, then runs the mock tests. The ownership library contains no Windows or MSFS API dependency. These tests do not establish compatibility with a private engine ABI.
