...
For an established camera pair, one bounded read-only transaction inspects the manager identity, view pool, entry table and both views. Private object reads share allocation identity and metadata for the pages actually requested, never field contents. `QueryVirtualMemoryInformation` supplies allocation identity and bounds; `QueryWorkingSetEx` supplies current protection only for valid resident pages. Unavailable page information falls back to the original `VirtualQueryEx` region checks. Every field is still read fresh and reread by the trace. The owned-view traces and the view pool read each object's known extent in one exact read: from its first to the end of its last observed field, such as an entry's identity fields or a handle's control record. Fields are served from that read. The reread repeats the span and compares only the bytes each field observed, because engine data between those fields can change every frame. A stale handle's control record is read whole, but its payload is never followed. Fields that are read only when an optional output is complete, such as the Bitmap output dimensions, keep their own reads. Allocation identities and every recorded page protection receive fresh endpoint checks before results are accepted. No cache survives a native engine call or a later frame. Changed ownership or intervening lifecycle work prevents reuse of the provisional pair inspection; the ordinary fresh inspection and refusal checks apply.
...
The bridge derives the rate it actually requests from the saved `camera_rate` without rewriting it. **Parked floor:** while public GROUND VELOCITY stays below 0.2 kt for 3 s the schedule runs at `parked_rate` (default 5; 0 disables); a sample at or above 0.35 kt restores the saved rate at once (a parked A350 reads 0.00–0.09 kt; 0.4 kt is an aircraft still rolling), 0.2–0.35 kt holds the current state so jitter around the edge cannot step the rate, and re-parking needs the full 3 s again so a creeping taxi start cannot flap. Missing or stale speed telemetry counts as moving. The views stay open and keep pulsing while parked; only `configure()` changes, which retains live deadlines. **Useful maximum:** the requested rate is capped at the lower of the profile's measured PFD refresh (`pfd_refresh_hz`, 0 = not measured) and the camera-manager ceiling of 15, because the 0.9.35 rate sweep showed the manager at 42–47 updates/s with inspections on 93–100 % of updates from rate 15 on. Rates above the cap are reported as capped in the companion status and `bridge.log` (`rate=… saved_rate=… useful_rate=… rate_limit=… parked=…`), not honoured. Live effect on frametime is not established by the local tests.

Source: [camera integration](../src/camera/probe.cpp), [pose conversion](../src/camera/body_pose_math.hpp), [mounts](../src/camera/aircraft_mounts.hpp), [schedule](../src/camera/render_schedule.hpp), [rate policy](../src/shared/camera_rate_policy.hpp).
...
Every profile also keeps a retained **PLEASE WAIT** page: black, with the text centred at 1.5 times the GS glyph size (30 working-image pixels tall). The Airbus profiles draw it in the GS speed colour; `waiting_white_text` draws it white on the PMDG 777, which has no GS readout. A separate compositor instance draws it without any camera input, into its own stable buffer and typed patches, on the same private-submission timeline as the camera output. It is drawn again only when its text colour or the requested patch set changes. A recording deferred by queue contention is kept, and only its submission is retried on the next service pass. Each display path (queue copy, barrier copy, recording-end shader) chooses per side between the camera image and this page. A side shows the page for its first 750 ms after it is admitted for display writes (`WaitingPageTimer`), and afterwards whenever no current camera composition exists or the latest one is more than one second old. The page never admits a side: cutoff, disconnect, identity, failed-start, degraded-graphics and native guards still leave the display untouched. A failure to build the page keeps the previous behaviour of drawing nothing until the camera image exists.
