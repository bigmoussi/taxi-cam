...
Profiles with `waiting_page` (all Airbus profiles; not the PMDG 777) also keep a retained **PLEASE WAIT** page: black, with the text centred at 1.5 times the GS glyph size (30 working-image pixels tall) in the GS speed colour. A separate compositor instance draws it without any camera input, into its own stable buffer and typed patches, on the same private-submission timeline as the camera output. It is drawn again only when the GS colour or the requested patch set changes. Each display path (queue copy, barrier copy, recording-end shader) chooses per side between the camera image and this page. A side shows the page for its first 1.5 seconds after it is admitted for display writes (`WaitingPageTimer`), and afterwards whenever no current camera composition exists or the latest one is more than one second old. The page never admits a side: cutoff, disconnect, identity, failed-start, degraded-graphics and native guards still leave the display untouched. A failure to build the page keeps the previous behaviour of drawing nothing until the camera image exists.
