...
Notes contain installation guidance, GPLv3 licence information, links to the exact corresponding source and build logs, validation scope, and a rollup of merged pull requests and first-parent headlines after the last release tag on `main`. Unpublished builds after that tag are included. Duplicate pull-request numbers and conventional `ci:` / publish-workflow-only changes are omitted. The first release includes the available first-parent history. The updater continues to see only published GitHub releases for those tags, never pull-request artifacts or an untagged `main` build.
