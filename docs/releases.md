...
Notes contain installation guidance, GPLv3 licence information, links to the exact corresponding source and build logs, validation scope, and a rollup of merged pull requests and first-parent headlines since the latest `v*-build.N` tag reachable on `origin/main` (or `main`). Unpublished builds after that tag are included. Duplicate pull-request numbers and conventional `ci:` / publish-workflow-only changes are omitted. The first release includes the available first-parent history.
