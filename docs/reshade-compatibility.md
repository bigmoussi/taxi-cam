...
For now, start Taxi Cam at the main menu before loading the flight. If it was attached late and targets remain missing, keep Taxi Cam running and use **Restart Flight**; this may recreate the required objects. A simulator restart with the companion already running also gives it a fresh opportunity to observe creation. Automatic startup helps arrange early attachment but does not implement late-resource discovery. **Auto-connect Off** followed by **Connect** after the aircraft has already created its display textures hits the same limit: Overview PFD dropdowns stay empty and Refresh finds nothing until those resources are recreated under observation. A nonzero capture/composition count cannot establish successful PFD routing or presentation. See [PFD-copy diagnostics](runtime-reference.md#pfd-copy-diagnostics) for the separate delivery counters.
