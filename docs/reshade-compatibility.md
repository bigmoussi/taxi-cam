...
The bridge observes resource creation and later render-target-view creation. After a late attach it also admits already-created display textures the first time a barrier or copy sees them, and it can recover the bound RTV handle when that unique resource is the next OM target on the same list. That path is a short one-shot window: each pointer is observed once, then a cheap already-seen check, then the extra work is disabled entirely (one relaxed flag load) after two PFD candidates and their handles exist. Three seconds stops looking only while the list is still empty; a filled list keeps association a little longer so stamps and calibration can appear. It does not walk the device heap and does not add locks, allocations or logging on the steady-state Present/barrier/copy/bind path.

Starting after the aircraft has loaded can still miss targets when instruments never transition or bind during that window, or when several render targets share one command list before OM. Overview says **waiting for cockpit displays** only while the PFD list is empty. Once candidates appear, ready/heading follow the list; a dominant-activity profile with exactly two learned displays adopts that pair so calibration has targets. Keep Taxi Cam running and look again after the next cockpit draw; use **Restart Flight** only if the list stays empty. Restarting the companion does not reconstruct missing GPU objects. A nonzero capture/composition count cannot establish successful PFD routing or presentation. See [PFD-copy diagnostics](runtime-reference.md#pfd-copy-diagnostics) for the separate delivery counters.
