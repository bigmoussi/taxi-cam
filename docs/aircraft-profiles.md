...
Texture admission requires 768 x 1024, exactly one mip and supported RGBA/BGRA views. The detected display group consists of eight active RGBA8 typeless resources (format 27), with the last allocation assigned left and the third-last assigned right. The automatic rule requires that complete group, unchanged membership and activity on every member across three one-second windows. It permits gaps in resource IDs and does not use fixed IDs. Missing, extra, changed or incompletely tracked resources prevent automatic selection. Structural changes withdraw automatically assigned sides while preserving sticky manual selections. When both auto-detected identities are destroyed, the next confirmed pair may be adopted again without reloading the profile. Sticky manual assignments still require choosing both-Auto or reselecting the aircraft profile. Pauses alone do not remove existing identities. The activity-ranking rule remains in use for FBW A380 and A350. Local detector tests cover both reference inventories; automatic iniBuilds A380 selection remains unverified in the simulator. Own-device GPU fixtures check display regions and preserved lower rows; hotkeys, AA, turns, cutoff and reload also require live verification.
