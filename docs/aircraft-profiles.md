...
| PMDG 777-300ER | `pmdg-777-300er` | Same `DUS` layout as 200ER | Same pane sizes; nose mount copied from 200ER |
...
Ground speed is **off** for these profiles only: the live path skips the GS overlay entirely (no readout and no black GS box). The GS font fixture still enables the panel on every catalog profile, including 777, and keeps the default panel origin so padding checks stay strict. Reference guides stay off; the companion disables that settings page for 777. Daytime exposure stays the shared default of **-11.5 EV**. Settings are stored per type in `pmdg-777.ini`, `pmdg-777-300er.ini` and `pmdg-777f.ini`. PFD refresh is not measured (`pfd_refresh_hz` 0).

Published mount defaults from live 777-200ER calibration: nose **0 / -2 / 16 m, pitch/yaw -18 / 0 degrees, lens 1 rad**; left wing **-6 / 1.5 / -28 m, -5 / -12 degrees, 0.6 rad**; right wing **6 / 1.5 / -28 m, -5 / 12 degrees, 0.6 rad**. The 300ER and 777F profiles copy that nose mount; wing mounts stay these published values until those types are calibrated separately. They are not a metric clearance calibration. Live framing and lighting still need an in-simulator check.
