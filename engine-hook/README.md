# Win64 camera-update observer experiment

This directory contains an isolated **vtable-slot observer hook** and a standalone mock-host validation. It has no engine addresses, process discovery, remote memory operations, camera creation calls or simulator installation step. The default observer is a no-op. The hook supplies the original RCX value to an optional observer, then tail-jumps to the original target without declaring that target's signature.
...
```powershell
& .\engine-hook\build.ps1
```

The script uses the existing pinned LLVM-MinGW `llvm-mingw-20260908-ucrt-x86_64` dependency from the parent native-probe directory. It compiles with C++20, `-O2 -Wall -Wextra -Werror`, and disables AVX generation for these local sources. It creates only ignored files under this directory's `build/`:
